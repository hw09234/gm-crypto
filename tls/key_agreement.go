...
func (ka rsaKeyAgreement) generateServerKeyExchange(config *Config, cert *Certificate, enccert *Certificate, clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
	return nil, nil
}
...
func (ka *ecdheKeyAgreement) generateServerKeyExchange(config *Config, cert *Certificate, enccert *Certificate, clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
...
// eccSM2KeyAgreement
type eccSM2KeyAgreement struct {
	isRSA      bool
	version    uint16
	privateKey []byte
	curveid    CurveID
...
func (ka *eccSM2KeyAgreement) generateServerKeyExchange(config *Config, signCert *Certificate, enccert *Certificate,
	clientHello *clientHelloMsg, hello *serverHelloMsg) (*serverKeyExchangeMsg, error) {
	digest := ka.hashForServerKeyExchange(clientHello.random, hello.random, enccert.Certificate[0])
...
func (ka *eccSM2KeyAgreement) processClientKeyExchange(config *Config, cert *Certificate, ckx *clientKeyExchangeMsg, version uint16) ([]byte, error) {
...
func (ka *eccSM2KeyAgreement) processServerKeyExchange(config *Config, clientHello *clientHelloMsg, serverHello *serverHelloMsg, cert *x509.Certificate, skx *serverKeyExchangeMsg) error {
	if len(skx.key) <= 2 {
		return errServerKeyExchange
	}
...
func (ka *eccSM2KeyAgreement) hashForServerKeyExchange(slices ...[]byte) []byte {
...
func (ka *eccSM2KeyAgreement) generateClientKeyExchange(config *Config, clientHello *clientHelloMsg, cert *x509.Certificate) ([]byte, *clientKeyExchangeMsg, error) {
	preMasterSecret := make([]byte, 48)
	preMasterSecret[0] = byte(clientHello.vers >> 8)
	preMasterSecret[1] = byte(clientHello.vers)
	_, err := io.ReadFull(config.rand(), preMasterSecret[2:])
